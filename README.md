...
"Не перевірено" on the automation's page, not an error. `/skills/import-github`
(single-repo and shared-folder bulk import alike) reads the same file from a skill's own
repo — the shield-icon badge (`src/templates/_security_chip.html`) on both automation
and skill cards shows one of four states: not reviewed (grey), clean (green), open
Medium findings (amber), open High findings (red), with the finding counts spelled out
in text, not just color. An
