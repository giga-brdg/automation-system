...
PHASES = [
    {
        ...
        "questions": [
            ...
            {"key": "target_metric_type", "label": "Які метрики важливі для цієї автоматизації? (можна декілька)",
            ...
            {"key": "baseline_cycle_minutes",
             "label": "Скільки хвилин зараз займає один цикл цього процесу вручну? "
                       "(заміряй один реальний цикл від початку до кінця, а не оціни на око; "
                       "якщо є AUTOMATION_REQUEST.md з цією цифрою — відштовхуйся від неї)",
             "type": "number"},
            {"key": "baseline_frequency_per_month",
             "label": "Скільки разів на місяць це відбувається?",
             "type": "number"},
            {"key": "target_cycle_minutes",
             "label": "Скільки хвилин очікується на один цикл після автоматизації? "
                       "(0, якщо повністю автоматично)",
             "type": "number"},
            {"key": "target_frequency_per_month",
             "label": "Скільки разів на місяць очікується після автоматизації? "
                       "(лиши порожнім, якщо частота не зміниться)",
             "type": "number"},
            {"key": "manual_reduction_target",
             "label": "Якщо серед метрик вище є конверсія, якість або витрати — яка конкретна ціль по них? "
                       "(число по кожній — в.п. конверсії, % витрат тощо; час вже пораховано вище)",
             "type": "textarea"},
            {"key": "success_check",
